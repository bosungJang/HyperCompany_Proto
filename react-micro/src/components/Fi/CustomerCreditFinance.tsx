...
const data = Array(17)
  ...
  .map(() => ({
    date: "2022.01.01",
    voucherNum: "20220111-005",
    briefs: "외상매출금 어음 회수",
    accountSubject: "받을 어음",
    OcurredAmount: 1000000,
    repaymentAmount: 0,
    balance: 1000000,
  }));

const CustomerCreditAndFinancialManagement = () => {
  /*Tabs */
  const [Tabs, setTabs] = React.useState("1");
  const [sumOA, setSumOA] = React.useState(0);
  const [sumRA, setSumRA] = React.useState(0);
  const [sumBL, setSumBL] = React.useState(0);
  /*Tabs */
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      sumOA += data[i].OcurredAmount;
      setSumOA(sumOA);

      sumRA += data[i].repaymentAmount;
      setSumRA(sumRA);

      sumBL += data[i].balance;
      setSumBL(sumBL);
    }
  }, []);
                                    ...
                                    >
                                      {data.map((item) => (
                                        ...
                                        >
                                          ...
                                          <td>{item.briefs}</td>
                                          <td>{item.accountSubject}</td>
                                          <td>{item.OcurredAmount}</td>
...
export default CustomerCreditAndFinancialManagement;
