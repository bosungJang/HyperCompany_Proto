...
export default function EvaluationPlanCreate() {
  /*BottomBar */
  ...
  const Card = (props: any) => {
    ...
    return (
      <StyledDiv style={{ height: 278 }}>
        ...
        <CheckField>
          <HcCheckBox checked={true} onChange={(e) => {}} />
          <Name>역량</Name>
          <HcTextFieldLabel
        ...
        >
          <svg
            onClick={() => {
              setSelected(selected.filter((obj: any) => obj.key !== key));
            }}
            width="24"
            height="24"
            viewBox="0 0 24 24"
  ...
  return (
    <>
      <ComponentWrapper style={{ display: "block", minHeight: 1300 }}>
        ...
        >
          <div style={{ display: "flex" }}>
            <div style={{ marginRight: 80, display: "block", width: 360 }}>
              ...
              <HcDateRangePicker />
              <div style={{ marginTop: 56, display: "flex", marginBottom: 10 }}>
                <HcSelect titleName="평가대상자" style={{ width: 160 }}>
                  <option>조직 개편일</option>
                </HcSelect>
                <HcSelect titleName="" style={{ width: 120, marginLeft: 10 }}>
                  <option>조직</option>
                </HcSelect>
                <HcSelect titleName="" style={{ width: 300, marginLeft: 10 }}>
                  <option>조직 선택</option>
                </HcSelect>
              </div>
              <HcTagNoInput
        ...
        >
          <Title style={{ marginBottom: 23 }}>평가 단계 설정</Title>
          <div style={{ marginBottom: 26 }}>
            ...
            >
              <HcRadioButton value="true">
                <span style={{ marginRight: 26, marginLeft: 8 }}>불러오기</span>
              </HcRadioButton>
              <HcRadioButton value="false">
                <span style={{ marginLeft: 8 }}>직접 설정</span>
              </HcRadioButton>
            </HcRadioGroup>
          </div>
          <div style={{ display: "block", marginLeft: -16 }}>
            ...
            >
              <ul style={{ padding: 0, margin: 0 }}>
                {selected.map((item: any) => (
                  <li key={item.key} style={{ marginBottom: 20 }}>
                    {console.log("created:" + item.key)}
                    <Card key={Number(item.key)} />
                  </li>
                ))}
              </ul>
            </div>
            <div
              style={{
                display: "flex",
                marginTop: selected.length > 0 ? 0 : 32,
              }}
            >
              <HcButton
                size="medium"
      ...
      <HcContentPopup
        ...
        style={{ right: 30 }}
      >
        <HcTableContainer style={{ width: 886, maxHeight: 491 }}>
          <HcTable style={{ width: "unset", tableLayout: "fixed" }}>
            <thead>
              <tr>
                <th style={{ minWidth: 243 }}>기타 평가명</th>
                <th style={{ minWidth: 120 }}>직무</th>
                <th style={{ minWidth: 400 }}>설명</th>
                <th style={{ minWidth: 120 }}>생성일시</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>
                  <a style={{ color: "#257CFF", textDecoration: "underLine" }}>
                    기타 평가명
                  </a>
                </td>
                <td>기획</td>
