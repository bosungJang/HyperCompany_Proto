...
export default function () {
  ...
  const [mail, setMail] = useState(
    ...
    [구성원 이름]님의 현재 사용 가능한 잔여 연차는 [15일]임을 알려드립니다.
    
    잔여 연차 [15]일에 대하여 2022년 7월 10일까지 연차 사용 일시를 지정하고 연차 휴가 계획서를 작성하여 
    
    인사팀으로 서면통하여 주실것을 촉구드립니다
    `
  );
  /*Toast */
  ...
  return (
    ...
    >
      ...
      <div style={{ float: "left", height: 40, marginBottom: 59 }}>
        <HcTitleTextField titleName="연차 사용 촉진" isBackIcon />
      </div>

      <NewHRInfo style={{ float: "left" }}>
        <SubTitle>연차 사용 촉진 대상자</SubTitle>
      ...
      <MainSendContainer>
        <SubTitle>촉진 메일 전송</SubTitle>
        <HcButton
          onClick={() => {
            message("촉진 메일 전송 완료하였습니다.", "test");
          }}
          styles="secondary"
          style={{
            marginTop: "18px",
      ...
      <HcMailPopup open={modalOpen} close={closeModal}  save={()=>{console.log("saved")}} header="촉진 메일 수정">
        <p
          style={{
            marginTop: "13px",
            fontSize: "14px",
